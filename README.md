...
- `/reporter.html` - Reporter Room: the newsroom. **News Intelligence** is its first tab — live news, per-asset headlines, scheduled catalysts, a compact market-reaction strip, Master News, desk status and newsroom controls — alongside the four generated desk tabs (Geopolitics, Economics, Stocks, Crypto). See [News Intelligence tab](#news-intelligence-tab).
...
### Reporter prompts and refresh frequency

Settings -> Reporter manages the requests the four reporter desks generate with. A **prompt** is a named block of text bound to one desk (Geopolitics, Economics, Stocks/Markets, Crypto/Emerging Markets); each desk generates with whichever of its prompts is in use. Prompts can be added, retitled, duplicated, moved between desks and deleted, and `{date}` in the body is replaced with the generation date. Every desk also ships a default prompt that can be retitled, rewritten and reset but not deleted, so a desk can never be left with nothing to generate from — deleting the prompt a desk was using drops it back to that default. The library lives in the browser under `reporterPrompts:v1`; prompts saved under the older per-section keys are carried into it on first read.
...
`/reporter.html` is the newsroom, not a market dashboard. Its tab row is:

```text
[ 📰 News Intelligence ] [ 🌍 Geopolitics ] [ 🏦 Economics ] [ 📈 Stocks ] [ ₿ Crypto ]
```

The four desk tabs are the generated reports, unchanged. **News Intelligence**
...
- **`news` is a UI tab, never a report section.** The backend still has exactly
  ...
  reaches generation, broadcast routing, the generation log or Master News —
  and a generation is refused for anything that is not a desk.
- **Switching tabs swaps two views, it does not rebuild them.** Third-party
  panels stay mounted once activated, so leaving a desk and coming back costs
  no further embed loads.
- **The newsroom does not duplicate the trading pages.** If a panel answers
  "what should I trade?" it belongs on the Terminal Suite, the screeners or
...
- **Earnings are links, not a feed.** No embed provider offers an earnings
  calendar widget, and the page will not take on a new data dependency for one.
  The panel shows the scheduled catalysts already in the payload — marking
  times it cannot confirm — and links out for the earnings list.
- **Nothing third-party loads until it is needed.** Panels are declared, not
  ...
  `reporter-widgets.js` mounts one as it approaches the viewport, shows a
  placeholder while it loads, and replaces a panel that never produces a frame
  with "Market data unavailable", a Retry and a link to the source. A collapsed
  section fetches nothing, so Global Context costs nothing until opened. Market
  News repoints one frame between assets rather than holding eight.
- **A provider outage is contained.** Every panel fails on its own; the AI
  Reporter, generation, Broadcast and saved reports keep working with all of
  them down.
- **Fixed desk pages open straight into their report.** `/emerging-markets.html`,
  `/economics-top-10.html` and `/markets-top-10.html` share this template, so
  they drop the workspace and the News tab entirely and request no third-party
  panels.
- **Master News is ready for classification it does not have yet.** Cards show
  rank, headline, desk, that desk's last run and its cited source count.
  Impact, affected assets and themes render from an optional
...
- **Source** - CFTC Commitments of Traders, Legacy Futures Only, from the CFTC Public Reporting API (`publicreporting.cftc.gov`, dataset `6dca-aqww`). Free and public domain; one request covers every market. Weekly only: positions as of Tuesday, published Friday. That is the Weekly timeframe.
- **Daily** - exchange open interest per trading session from Databento (paid; `DATABENTO_API_KEY`). Each market is requested by parent symbol from the `statistics` schema (e.g. `GLBX.MDP3` `GC.FUT`; DXY from `IFUS.IMPACT` `DX.FUT`), and open interest is summed across every listed expiry. Δ 1D compares with the previous session, Δ 5D with five sessions back. Exchanges publish OI once per session, so no 4h/8h figure exists for these markets. COT positioning is weekly only and is disabled on Daily. Without a key, Daily is greyed out with the reason and Weekly is unaffected. `CROSS_MARKET_OI_DAILY_STALE_AFTER_DAYS` (default 4) marks a series that stopped advancing; the last good daily copy is kept like the weekly one.
...
- **What the number is** - `OI change, all expiries`: `((current − previous) / previous) × 100` of the CFTC's `open_interest_all` (every listed expiry combined) between two report dates. It is not front-month or continuous-contract (`ES1!`) OI, so it will not match a charting site's OI. Each row carries `observationDate`, `comparisonDate`, `retrievedAt`, `contractScope`, `valueStatus`/`statusReason`, `isStale`, `isFallback` and `identityVerified` (the CFTC report name is checked against `cftcName` in the config). ROLL covers any comparison touching a quarterly expiry window. Audit and validation steps: `docs/cross-market-oi-accuracy-audit.md`; `node scripts/oi-diagnostics.js --url <host> --cftc` prints the per-market trace and recomputes it from the CFTC directly.
- **Missing and stale** - a market absent from the report is `null` and leaves a gap in the shape, never 0. A market whose latest report is older than the others' is STALE. A report older than `CROSS_MARKET_OI_STALE_AFTER_DAYS` (default 11) is STALE. The last good report is saved to `DATA_DIR/cross-market-oi.json`; if the CFTC is unreachable the page shows it as CACHED.
...
`/rsi-matrix.html` (sidebar: TradeHunter → Screeners → RSI Matrix) shows RSI 14 on close for many markets across 1W / 1D / 4H / 1H at once, as a heatmap with columns = instruments and rows = timeframes plus AVG. It reads `GET /api/rsi-matrix` (`?force=1` is the Refresh button), served from `src/services/rsi-matrix/`.

- **Calculation** - the Signal Screener's own `rsi()` on closed bars only (`dropUnclosedCandle`). Cells use momentum bands (≥75 strong bullish … ≤25 strong bearish). AVG is the mean of every *enabled* timeframe and is reported only when all of them have a value; otherwise it shows `— (3/4)`. AVG ≥ 70 is flagged MTF overbought and ≤ 30 MTF oversold. These are RSI conditions, never BUY/SELL.
- **Instruments** - a registry separate from the Binance-only token universe: each row is a display label, a group (Cross-Market, Crypto / Dominance / Stablecoins), a provider and the provider's own symbol. Edited in **Settings → Screeners → RSI Matrix** (add, delete user rows, reorder, enable, relabel, change source, timeframes, reset) and saved to `DATA_DIR/rsi-matrix-settings.json`. A new or changed source is checked against its provider before it is saved; TradingView notation (`MEXC:MXUSDT`) and synthetic indices sent to an exchange are refused.
