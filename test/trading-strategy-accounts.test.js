...
test("the accounts overview reports equity and lifecycle for all seven", async () => {
  const lab = tempLab();
  const overview = await lab.strategyAccounts();

  assert.equal(overview.accounts.length, 7);
  assert.equal(overview.mode, "paper");
  assert.equal(
    new Set(overview.accounts.map((account) => account.stats.startingBalance)).size,
    1,
    "all seven accounts must begin with the same fair starting balance",
  );
